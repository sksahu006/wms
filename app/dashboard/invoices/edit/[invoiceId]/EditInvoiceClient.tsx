...
export default function EditInvoicePageClient({ invoice }: { invoice: any }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Add all required fields from the invoice object
    ...
    // Format dates in ISO format for Zod datetime validation
    if (invoice.date) {
      formData.append("date", new Date(invoice.date).toISOString())
    } else {
      formData.append("date", new Date().toISOString())
    }
    
    // Get due date from form and ensure it's in ISO format
    const dueDateInput = formData.get("dueDate") as string
    if (dueDateInput) {
      // Convert YYYY-MM-DD to ISO format by appending time
      const dueDate = new Date(`${dueDateInput}T00:00:00Z`)
      formData.append("dueDate", dueDate.toISOString())
    }
    
    // Calculate total amount if tax exists
    ...
    <div className="max-w-xl mx-auto py-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input id="amount" name="amount" defaultValue={invoice.amount} type="number" required />
        </div>
